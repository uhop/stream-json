...
- **JSONL** (**deprecated — slated for removal in a future major**): `jsonl/parser.js` and `jsonl/stringer.js` are thin re-exports of stream-chain's JSONL (the parser API was absorbed into stream-chain; dep is `^4.2.1`). The Node/Web wrappers delegate `.asStream`/`.asWebStream` to stream-chain's bundled `stream-chain/node/jsonl/*` and `stream-chain/web/jsonl/*` factories. Use stream-chain's JSONL directly. Rationale: stream-json is a JSON _token_ library; JSONL yields whole objects per line and belongs in stream-chain with the other substrate components.
