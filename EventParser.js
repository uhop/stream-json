...
var Transform = require("./pipes/EventTransform");
		...
		main: for(;;){
			switch(this._expect){
				...
				case "value":
					...
					switch(value){
						case "\"":
							this.pushValue({id: value, value: value});
							this._expect = "string";
							break;
						case "{":
							this.pushValue({id: value, value: value});
						...
						case "8":
						case "9":
							this.pushValue({id: "nonZero", value: value});
				...
				case "string":
					...
					if(value === "\""){
						...
						this.pushValue({id: "escapedChars", value: value});
					}else{
						this.pushValue({id: "plainChunk", value: value});
					}
					this._buffer = this._buffer.substring(value.length);
					break;
				// number chunks
				...
				case "number1": // [0-9]*
					...
					value = match[0];
					if(value){
						this.pushValue({id: "numericChunk", value: value});
						this._buffer = this._buffer.substring(value.length);
					}else{
						if(this._buffer){
							this._expect = "number2";
				...
				case "number2": // [\.eE]?
					...
					if(value === "."){
						...
						this.pushValue({id: "exponent", value: value});
