...
- **JSONL** (**deprecated — slated for removal in a future major**): `jsonl/parser.js` and `jsonl/stringer.js` are thin re-exports of `stream-chain/jsonl/*` (the parser API was absorbed into stream-chain 4.2.0). Use stream-chain's JSONL directly. Rationale: stream-json is a JSON _token_ library; JSONL yields whole objects per line and belongs in stream-chain with the other substrate components. `jsonlStringer` is still a `Transform` on Node; `jsonlStringer.asWebStream(options)` delegates to `stream-chain/jsonl/stringerWebStream` and returns a Web `TransformStream<T, string>`.
