 ...
 */
declare function streamValues<T = unknown>(options?: StreamBaseOptions): parser.TokenConsumer<streamValues.StreamValuesItem<T>>;

declare namespace streamValues {
  /**
   * An item emitted by `streamValues` — `KeyedValue<number, T>`: `key` is a
   * zero-based sequential counter, `value` the assembled top-level value.
   *
   ...
   * parameter through.
   */
  export type StreamValuesItem<T = unknown> = KeyedValue<number, T>;
