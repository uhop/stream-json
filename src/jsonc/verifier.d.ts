 ...
 *
 * Node-flavored entry: the returned factory has both `jsoncVerifier.asStream(options)`
 * (Node Duplex) and `jsoncVerifier.asWebStream(options)` (Web `{readable, writable}` pair) attached.
 *
...
declare namespace verifier {
  ...
  export interface JsoncVerifierOptions extends CoreJsoncVerifierOptions, DuplexOptions {}
  ...
  /** Creates a JSONC Verifier wrapped as a Node Duplex stream. */
