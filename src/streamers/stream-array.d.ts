 ...
 *
 * Node-flavored entry: the returned factory has both `streamArray.asStream(options)`
 * (Node Duplex) and `streamArray.asWebStream(options)` (Web `{readable, writable}` pair) attached,
...
declare namespace streamArray {
  ...
  /** Creates a streamArray wrapped as a Node Duplex stream. */
